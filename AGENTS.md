...
- `src/components/InformationUploadTab.tsx` - UI-only Dentalpedia upload workspace for Figma `6916:53548` and `7015:61602`. The video form and responsive article editor/live-preview view own only local presentation state; article imagery and editor icons live under `public/dentalpedia/`, and persistence remains disconnected until an Admin API contract is defined.
...
- 정보 업로드: lives directly below `시크릿 피드백` in the sidebar and reuses the `운영 현황` `Type=Dashboard.svg` icon. Its Figma `6916:53548` Dentalpedia form remains UI-only: the video/article pills and field values are local browser state, and 임시저장/게시하기 do not call an API. Keep the upload card fluid across the full available Admin canvas, reflow controls at narrow breakpoints, and use the tracked `Type=UploadCloud.svg` for the thumbnail drop zone.
