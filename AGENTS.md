...
- `src/components/InformationUploadTab.tsx` - 치카피디아 upload workspace for current video-editor Figma `7385:76126` and article-editor Figma `7015:61602`. The video view uses a responsive editor/live-preview split pane, persists and restores drafts, uploads the Home thumbnail plus an MP4/WebM file up to 50MB directly below the optional YouTube URL, and publishes visible or hidden videos through the protected Dentalpedia API contract; its shared types and validation live in `src/lib/dentalpedia-video.ts`. `src/components/DentalpediaArticleEditor.tsx` owns the article editor/live preview and persists drafts, immediate publications, scheduled publications, cover images, and Markdown body images. Article imagery and editor icons live under `public/dentalpedia/`.
...
- 치카피디아: lives directly below `시크릿 피드백` in the sidebar and reuses the `운영 현황` `Type=Dashboard.svg` icon. Current Figma `7385:76126` `영상` and Figma `7015:61602` `칼럼/게시글` both use authenticated `/api/v1/admin/dentalpedia/*` requests for draft persistence, editing, and publication. The video editor persists title/category/summary/tags/search keywords, duration, ChikaPedia/recommendation/Hero/Home exposure, Home order, exposure priority, immediate or scheduled publication, optional end date, and selected related-content IDs. It uploads its thumbnail through the shared signed JPG/PNG/WebP path and accepts either a signed direct MP4/WebM upload up to 50MB immediately below the URL field or a supported HTTPS YouTube URL; the API remains authoritative for object metadata, publication windows, ordering, and visibility. Article publishing additionally supports scheduling. Separate current video/article IDs are retained in browser local storage so each saved draft reopens after tab or page reload; the article `취소` action intentionally clears only its article pointer. Keep the editor/live-preview split fluid across the full available Admin canvas, reflow it to one column at narrow container widths, and use the tracked `Type=UploadCloud.svg` for the video file drop zone.
