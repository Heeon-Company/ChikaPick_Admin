...
- `src/lib/clinic-partnership-requests.ts` and `src/components/ClinicPartnershipRequestsTab.tsx` - Grouped external-clinic demand types/presentation plus the searchable, status-managed Admin workflow tab.
...
- 입점 신청 관리: patient-submitted demand for non-partner HIRA/Kakao clinics, grouped by external clinic instead of duplicated per requester. The tab shows status summary cards, server-backed clinic/address/phone search and status filtering, exact pagination, request counts and dates, requester history, safe external-place links, and an accessible responsive detail dialog. Provider-specific clinic IDs stay out of the primary clinic-information grid and remain available with copy feedback only inside the collapsed `기술 정보` disclosure. Unlocked Admin accounts can move the entire clinic request group through `접수`, `연락 중`, `입점 완료`, or `보류`, save a 1,000-character operating note, and rely on the API audit log for every change.
- 서비스 확대 요청 관리: lives directly below `시크릿 피드백` in the sidebar and reuses the `운영 현황` dashboard icon. Its `지역 서비스 요청` view reads exact request totals, recent-30-day demand, distinct requested districts, the top district, a paginated region/email directory, and the server-ranked top 10. Its `치과 제휴 요청` view reuses the grouped clinic-partnership directory and requester history while presenting the simplified Figma table and modal. Both count badges and metric cards come from the service-expansion Admin API rather than the current page of browser data. The adjacent `앱 문구 관리` view edits `header`, `content`, ordered active 시/도·시/군/구, `buttonInfo`, and `buttonText`; a 393px simulator mirrors the Client `서비스지역` hierarchy and updates as the form changes. `취소` restores the last server-loaded/saved form, while `저장` uses the protected atomic config endpoint and refreshes the form from its normalized response.
