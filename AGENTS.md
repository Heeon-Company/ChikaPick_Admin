...
- `src/components/InformationUploadTab.tsx` - Dentalpedia upload workspace for Figma `6916:53548` and `7015:61602`. The video form remains UI-only; `src/components/DentalpediaArticleEditor.tsx` owns the responsive article editor/live preview and persists drafts, immediate publications, scheduled publications, cover images, and Markdown body images through the protected Dentalpedia Admin API. Article imagery and editor icons live under `public/dentalpedia/`, while shared validation and API types live in `src/lib/dentalpedia.ts`.
...
- 정보 업로드: lives directly below `시크릿 피드백` in the sidebar and reuses the `운영 현황` `Type=Dashboard.svg` icon. Its Figma `6916:53548` video form remains UI-only, while Figma `7015:61602` `칼럼/게시글` uses authenticated `/api/v1/admin/dentalpedia/*` requests for signed direct JPG/PNG/WebP uploads up to 10MB, draft persistence, editing, immediate publishing, and scheduled publishing. The current article ID is retained in browser local storage so the same saved draft reopens after tab or page reload; `취소` intentionally clears that pointer and starts a new article. The API remains authoritative for Admin access, image signatures, field limits, canonical body-image references, slug uniqueness, and publication visibility. Keep the upload card fluid across the full available Admin canvas, reflow controls at narrow breakpoints, and use the tracked `Type=UploadCloud.svg` for the video thumbnail drop zone.
