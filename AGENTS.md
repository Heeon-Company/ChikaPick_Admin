...
- `src/components/InformationUploadTab.tsx` - Dentalpedia upload workspace for Figma `6916:53548` and `7015:61602`. The responsive video form persists and restores drafts, uploads thumbnail images, validates YouTube URLs, and publishes visible or hidden videos through the protected Dentalpedia Admin API; its shared types and validation live in `src/lib/dentalpedia-video.ts`. `src/components/DentalpediaArticleEditor.tsx` owns the article editor/live preview and persists drafts, immediate publications, scheduled publications, cover images, and Markdown body images. Article imagery and editor icons live under `public/dentalpedia/`.
...
- 정보 업로드: lives directly below `시크릿 피드백` in the sidebar and reuses the `운영 현황` `Type=Dashboard.svg` icon. Figma `6916:53548` `영상` and Figma `7015:61602` `칼럼/게시글` both use authenticated `/api/v1/admin/dentalpedia/*` requests for signed direct JPG/PNG/WebP uploads up to 10MB, draft persistence, editing, and publication. Video publishing accepts only supported HTTPS YouTube URL forms, uses the existing switch as public visibility, and publishes immediately; article publishing additionally supports scheduling. Separate current video/article IDs are retained in browser local storage so each saved draft reopens after tab or page reload; the article `취소` action intentionally clears only its article pointer. The API remains authoritative for Admin access, image signatures, field limits, YouTube URL normalization, canonical body-image references, slug uniqueness, and publication visibility. Keep the upload card fluid across the full available Admin canvas, reflow controls at narrow breakpoints, and use the tracked `Type=UploadCloud.svg` for the video thumbnail drop zone.
