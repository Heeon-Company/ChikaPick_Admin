...
- 치아톡 관리: UI-only Figma implementation using the same dashboard icon as `운영 현황`. The four summary cards, status pills, report-reason selector, and content search filter the current reference report rows locally until the Admin reporting API contract is introduced. The directory fills the fluid Admin canvas with responsive compact rails and 8px inner gutters based on the Figma table. The action rail alone stays at its natural 160px contract (`8px + 72px 상세 보기 + 80px`), and the content rail starts from the prior 45% allocation then absorbs only the width released by capping that action rail; the button uses auto left margin so its right edge remains exactly 80px from the table edge. Keep the action-cell padding selector scoped through the table and `td` so it outranks the generic table-cell padding rule. Only genuinely narrow viewports use the bounded horizontal table scroller, and the summary/filter controls reflow without widening the document. `상세 보기` opens the 468px right-side report drawer without dimming the underlying directory; 게시글 shows author/date, title/body, the tracked `Type=Image.svg` placeholder, sanctions, and report history, while 댓글 shows its original-post context, accented reported comment, author/date, and report history. Escape, the close control, or the outside layer closes the accessible modal drawer; the bottom moderation actions remain UI-only until the reporting API contract exists.
