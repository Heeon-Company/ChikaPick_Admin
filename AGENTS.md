...
- 치아톡 관리: reads the protected moderation list, metrics, report-detail, and audited action APIs with the current Admin bearer token. The list is currently a first-page launcher for detail inspection, not the production-complete moderation queue: server pagination plus the complete target/age/repeat/author filter UX remains tracked separately. The four live summary metrics use the Figma dashboard cards: one fluid four-column row with 112px white cards, 20px gaps/padding, 16px labels, and 32px values; the existing two-column and single-column narrow-screen breakpoints remain. The status/reason controls reload the protected list while text search filters only the loaded page. The directory retains the fluid Figma canvas and bounded narrow-screen scroller. `상세 보기` opens the 468px right-side drawer without dimming the directory: post reports follow Figma `6788:54403`, and comment/reply reports follow `6790:2269`, including the original-post context, compact report history, and fixed `문제없음` / `콘텐츠 삭제` footer. The former invokes audited `dismiss_report`; the latter confirms and invokes `hide_content`, which removes the target from public ChikaTalk without physically erasing audit evidence. Reuse one request UUID after an uncertain failure, disable duplicate actions while pending, close and refresh the live queue after success, and keep API errors in the drawer. Reporter UUIDs must never reach the UI; display only API-issued per-detail aliases, safe author nicknames, and short-lived evidence media URLs. Escape, the close control, or the outside layer closes the accessible drawer.
