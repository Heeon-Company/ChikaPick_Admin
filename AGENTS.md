...
- `src/components/InformationUploadTab.tsx` - 치카피디아 upload workspace for Figma `6916:53548` and `7015:61602`. The responsive video form persists and restores drafts, uploads the Home thumbnail plus an MP4/WebM file up to 50MB, retains YouTube URL fallback, and publishes visible or hidden videos through the protected Dentalpedia API contract; its shared types and validation live in `src/lib/dentalpedia-video.ts`. `src/components/DentalpediaArticleEditor.tsx` owns the article editor/live preview and persists drafts, immediate publications, scheduled publications, cover images, and Markdown body images. Article imagery and editor icons live under `public/dentalpedia/`.
...
- 치카피디아: lives directly below `시크릿 피드백` in the sidebar and reuses the `운영 현황` `Type=Dashboard.svg` icon. Figma `6916:53548` `영상` and Figma `7015:61602` `칼럼/게시글` both use authenticated `/api/v1/admin/dentalpedia/*` requests for draft persistence, editing, and publication. The video editor persists the Figma Home-card category/title/visibility/recommendation/order fields, uploads its thumbnail through the shared signed JPG/PNG/WebP path, and accepts either a signed direct MP4/WebM upload up to 50MB or a supported HTTPS YouTube URL; the API remains authoritative for object metadata, publication fields, ordering, and visibility. Article publishing additionally supports scheduling. Separate current video/article IDs are retained in browser local storage so each saved draft reopens after tab or page reload; the article `취소` action intentionally clears only its article pointer. Keep the upload card fluid across the full available Admin canvas, reflow controls at narrow breakpoints, and use the tracked `Type=UploadCloud.svg` for the video file drop zone.
