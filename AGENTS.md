...
- 치아톡 관리: reads the protected moderation list, metrics, and report-detail APIs with the current Admin bearer token. The list is currently a first-page launcher for detail inspection, not the production-complete moderation queue: server pagination plus the complete target/age/repeat/author filter UX remains tracked separately. The four summary cards show live safe metrics, and the status/reason controls reload the protected list while text search filters only the loaded page. The directory retains the fluid Figma canvas and bounded narrow-screen scroller. `상세 보기` opens the 468px right-side drawer without dimming the directory and shows the immutable reported snapshot, current content or missing state, safe author nickname, original-post context for comments/replies, short-lived evidence image URL, current sanctions, author enforcement history, per-target action history, and related reports. Reporter UUIDs must never reach the UI; display only the API-issued per-detail aliases. Escape, the close control, or the outside layer closes the accessible drawer. Do not add moderation mutation controls to this read-only detail increment without wiring audited API actions, idempotent request IDs, confirmation/reason UX, refresh, and failure recovery together.
